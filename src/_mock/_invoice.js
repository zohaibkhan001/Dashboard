...
export const INVOICE_SERVICE_OPTIONS = [...Array(8)].map((_, index) => ({
  ...
  paymentMethod: _mock.paymentMethod(index),
}));
...
const ITEMS = [...Array(3)].map((__, index) => {
  ...
  return {
    ...
    paymentMethod: INVOICE_SERVICE_OPTIONS[index].paymentMethod,
  };
});
...
export const _invoices = [...Array(20)].map((_, index) => {
  ...
  //   (index % 2 && 'paid') || (index % 3 && 'pending') || (index % 4 && 'overdue') || 'draft';
    ...
    // const status =
    // (index % 2 && 'paid') || (index % 4 && 'pending') || 'paid';

    const status = 'paid';
    
    const paymentMethod = _mock.paymentMethod(index + 1);
